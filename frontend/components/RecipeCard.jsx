...
const RecipeCard = ({ recipe, variant = "default" }) => {
  const getRecipeData = () => {
    // For MealDB recipes (catigory/cuisine pages)
    if (recipe.strMeal) {
      return {
    ...
    return (
      <Link href={data.href}>
        <Card className="rounded-none overflow-hidden border-stone-200 hover:shadow-xl hover:translate-y-1 transition-all duration-300 cursor-pointer group pt-0">
          ...
          <CardHeader>
            <CardTitle className="text-lg font-bold teststone-900 group-hover:text-orange-600 transition-colors line-clamp-2">
              {data.title}
            </CardTitle>
          </CardHeader>
        </Card>
